...
## 23.05.11 11주차  
***Shared State***  
ㆍ하위 `컴포넌트` 가 공통된 부모 컴포넌트의 state를 사용하는것을 의미  

***State 끌어올리기***  
ㆍ하위 `컴포넌트`의 state를 공통된 부모 `컴포넌트`로 끌어올려 공유하는 방식을 의미  
ㆍ이는 Shared State를 응용한것  

***장점***  
ㆍ`단방향 데이터 흐름` 이기 때문에 코드 수정이 현저히 적어지고, 코드의 흐름을 읽기 쉬워짐  
ㆍ각 컴포넌트 모두가 `state`에 값을 가진게 아닌 상위 공통 컴포넌트로 `공유` 하기 때문에 효율적임  
